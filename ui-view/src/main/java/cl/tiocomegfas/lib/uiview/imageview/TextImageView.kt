package cl.tiocomegfas.lib.uiview.imageview
...
import cl.tiocomegfas.lib.uiview.R
...
class TextImageView @JvmOverloads constructor(
    ...
    init {
        ...
        val typedArray = context.theme.obtainStyledAttributes(
            attrs, R.styleable.TextImageView,
            0, 0
        )
        mBorderColor =
            typedArray.getColor(R.styleable.TextImageView_borderColor, Color.TRANSPARENT)
        mBorderWidth =
            typedArray.getDimensionPixelSize(R.styleable.TextImageView_borderWidth, 0).toFloat()
        mCircleColor = typedArray.getColor(
            R.styleable.TextImageView_circleColor,
            Color.TRANSPARENT
        )
        mTextSize = typedArray.getDimensionPixelSize(
            R.styleable.TextImageView_textSize,
            (24 * context.resources.displayMetrics.density).toInt()
        )
        mTextColor = typedArray.getColor(R.styleable.TextImageView_textColor, Paint().color)
        mTextStyle = typedArray.getInt(R.styleable.TextImageView_textStyle, FILL)
        mLetterSpace = typedArray.getFloat(R.styleable.TextImageView_letterSpace, 0f)
        if (mLetterSpace < 0 || mLetterSpace > 1) mLetterSpace = 0f
        mShadow = typedArray.getBoolean(R.styleable.TextImageView_shadow, false)
        mOffset = typedArray.getDimensionPixelSize(R.styleable.TextImageView_offset, mTextSize / 3)
            .toFloat()
        hasText = typedArray.hasValueOrEmpty(R.styleable.TextImageView_text)
        mText = if (hasText) typedArray.getText(R.styleable.TextImageView_text) else ""
        mMaxLength = typedArray.getInt(R.styleable.TextImageView_maxLength, Int.MAX_VALUE)
        mCircleSize =
            typedArray.getInt(R.styleable.TextImageView_circle_size, CircleSize.WRAP_TEXT.ordinal)
        typedArray.recycle()
        setUpPaint()
    }
